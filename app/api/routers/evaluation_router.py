...
@router.post("/evaluate", summary="Evaluate model performance", description="Start a model evaluation on a specific dataset.")
async def evaluate_model(
    ...
    samples: Optional[int] = Query(None, description="Number of samples to use (None for all)"),
    ...
    def run_evaluation():
        try:
            logger.info(f"Starting model evaluation on {dataset} ({split}) with samples={samples}")
            ...
            if metrics:
                ...
                logger.info(f"Results: Accuracy={metrics['accuracy']:.4f}, F1={metrics['macro_f1']:.4f}")
            else:
